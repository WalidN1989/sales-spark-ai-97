-- Payment Follow-up module: track money + logistics follow-ups (PDCs,
-- collections, PO/payment-advice, demo units, consignment) with an activity
-- timeline. last_activity_at is kept current by a trigger so the Wed/Fri
-- reminder assistant can query "stale" items. Shared team data (RLS allows any
-- authenticated user); Edge Functions use the service role.
...
-- RLS (shared across authenticated org users)
DO $$
DECLARE t text;
BEGIN
...
-- Seed the currently-tracked open items (only if the table is empty).
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.payment_followups) THEN
    INSERT INTO public.payment_followups (company_name, category, reference, title, amount_aed, quantity, unit_sku, due_date, sent_date, status, owner) VALUES
      ('Techsys Technology LLC', 'pending_pdc', '26391', 'Inv. 26391 PDC', 13230, NULL, NULL, '2026-09-18', NULL, 'open', 'Walid'),
