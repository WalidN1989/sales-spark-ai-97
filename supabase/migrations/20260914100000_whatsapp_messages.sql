-- WhatsApp conversations (Twilio). One row per message, linked to a lead. The
-- inbound webhook (service role) writes incoming messages; the app sends
-- outbound. RLS: a rep sees messages for leads they can access; a manager sees
-- all (including inbound from numbers not yet matched to a lead).
